...
class Consumidor extends React.Component {
	...
	filtrarLista = () => {
		console.log()
		return this.state.listaDeProdutos.filter(produto => {
			if(this.state.categoriaSelecionada === "Todas as Categorias") {
				return true
			}
			
			if(produto.category === this.state.categoriaSelecionada) {
				return true
			}
